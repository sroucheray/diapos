    ...
    $.get('albums.xml', function(data){
        $(data).find('item').each(function(i, item){
            var $item = $(item),
                ...
                parsedYear,
                linkMatch = $item.find('link').text().match(linkRegEx),
                guidMatch = $item.find('guid').text().match(guidRegEx),
                promise;

            it.displayTitle = titleMap[it.title] || "";
            it.id = encodeURI(it.title);
